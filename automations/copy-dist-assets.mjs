...
function hasPathSegment(relative, segment) {
  return relative.split(path.sep).includes(segment);
}
...
async function copyAssets(directory) {
  ...
  for (const entry of entries) {
    ...
    if (
      hasPathSegment(relative, ".raxode")
      || relative.startsWith(`reports${path.sep}`)
      || relative.startsWith(`frontend${path.sep}tui${path.sep}memory${path.sep}generated${path.sep}`)
    ) {
      continue;
    }
