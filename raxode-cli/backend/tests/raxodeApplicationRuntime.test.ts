...
test("raxode application runtime exposes current workspace path contract for tool calls", async () => {
  const workspace = await mkdtemp(path.join(os.tmpdir(), "raxode-workspace-paths-"));
  const processCwdArtifact = path.resolve("server.js");
  const events: unknown[] = [];
  ...
  try {
    ...
    assert.equal(await readFile(processCwdArtifact, "utf8"), "relative\n");
    const aliasCode = findToolEvent(events, "code-alias-path", "failed");
    const aliasCodeMetadata = aliasCode?.metadata?.resultMetadata as Record<string, unknown> | undefined;
    assert.equal(aliasCodeMetadata?.errorCode, "TARGET_OUT_OF_SCOPE");

    const absoluteCode = findToolEvent(events, "code-absolute-path", "failed");
    const absoluteCodeMetadata = absoluteCode?.metadata?.resultMetadata as Record<string, unknown> | undefined;
    assert.equal(absoluteCodeMetadata?.errorCode, "TARGET_OUT_OF_SCOPE");

    const relativeShell = findToolEvent(events, "shell-relative-cwd", "failed");
    const relativeShellMetadata = relativeShell?.metadata?.resultMetadata as Record<string, unknown> | undefined;
    assert.equal(relativeShellMetadata?.errorCode, "PROVIDER_REJECTED");

    const aliasShell = findToolEvent(events, "shell-alias-cwd", "failed");
    const aliasShellMetadata = aliasShell?.metadata?.resultMetadata as Record<string, unknown> | undefined;
    assert.equal(aliasShellMetadata?.errorCode, "PROVIDER_REJECTED");
  } finally {
    await rm(processCwdArtifact, { force: true });
...
test("raxode application runtime marks background service launches as unverified until reachability is checked", async () => {
  ...
  assert.equal(completedToolEvent.metadata?.resultMetadata?.serviceStatus, undefined);
  assert.equal(completedToolEvent.metadata?.resultMetadata?.verificationStatus, undefined);
  assert.equal(completedToolEvent.metadata?.resultMetadata?.serviceHandle, undefined);
  assert.equal(completedToolEvent.metadata?.resultMetadata?.nextRequiredAction, undefined);
  assert.ok(
    completedToolEvent.metadata?.humanResultSummary?.some((line) => /Launched/u.test(line)),
  );
});
...
test("raxode bapr carries application approval into detached shell TAP approval fields", async () => {
  ...
  assert.match(JSON.stringify(completedToolEvent.metadata?.humanResultSummary), /Launched/u);
  assert.equal((completedToolEvent.metadata?.resultMetadata as Record<string, unknown> | undefined)?.serviceVerificationStatus, undefined);
  assert.equal((completedToolEvent.metadata?.resultMetadata as Record<string, unknown> | undefined)?.processId, undefined);
});
