...
function searchWorkspaceEntries(
  ...
  const best: WorkspaceIndexSearchResult[] = [];
  const compareSearchResult = (left: WorkspaceIndexSearchResult, right: WorkspaceIndexSearchResult) => {
  ...
  for (const entry of entries) {
    ...
    best.sort(compareSearchResult);
    if (best.length > boundedLimit) {
      best.length = boundedLimit;
    }
  }
