...
async function exchangeAuthorizationCode(params: {
  ...
  if (!response.ok) {
    ...
    throw new RaxodeConfigError(`OpenAI token exchange failed: ${response.status} ${detail || response.statusText}`);
  }
  const payload = await response.json() as OpenAITokenResponse;
  if (!payload.access_token || !payload.refresh_token || !payload.id_token) {
    throw new RaxodeConfigError("OpenAI token exchange response is missing required tokens.");
  }
  return {
...
async function refreshTokenWithAuthority(refreshToken: string, issuer = OPENAI_OAUTH_ISSUER, clientId = OPENAI_OAUTH_CLIENT_ID): Promise<OpenAITokenResponse> {
  ...
  if (!response.ok) {
    const detail = await response.text();
    throw new RaxodeConfigError(`OpenAI token refresh failed: ${response.status} ${detail || response.statusText}`);
  }
  return await response.json() as OpenAITokenResponse;
}

function shouldAttemptRefresh(profile: RaxodeAuthProfile, force = false): boolean {
...
function upsertAuthProfile(authFile: RaxodeAuthFile, profile: RaxodeAuthProfile): void {
...
function ensureOfficialOpenAIProviderProfile(configFile: RaxodeConfigFile): void {
  const existing = configFile.profiles.find((entry) => entry.id === OPENAI_OFFICIAL_PROVIDER_PROFILE_ID);
  const corePlan = loadRaxodeRolePlan("core.main");
  if (existing) {
...
function persistOfficialOpenAIProfile(params: {
  ...
  const profile: RaxodeAuthProfile = {
  ...
  writeRaxodeAuthFile(authFile, fallbackDir);
  writeRaxodeConfigFile(configFile, fallbackDir);
...
function persistApiKeyToDefaultProfile(apiKey: string, fallbackDir = process.cwd()): OpenAIAuthStatus {
  ...
  return {
    ...
    activeProviderProfileId: loadRaxodeConfigFile(fallbackDir).providerSlots.openai,
  ...
  return await new Promise<Required<OpenAITokenResponse>>((resolve, reject) => {
    const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      try {
        ...
        if (callbackState !== state) {
          ...
          reject(new RaxodeConfigError("OpenAI OAuth callback state mismatch."));
          server.close();
          return;
        }
        ...
        if (errorCode) {
          ...
          reject(new RaxodeConfigError(`OpenAI OAuth callback failed: ${errorDescription ?? errorCode}`));
          server.close();
          return;
        }
        ...
        if (!code) {
          ...
          reject(new RaxodeConfigError("OpenAI OAuth callback did not include an authorization code."));
          server.close();
          return;
        }
        ...
        res.end("<html><body><h2>Raxode login complete</h2><p>You can return to the terminal.</p></body></html>");
        resolve(tokens);
        server.close();
      } catch (error) {
        res.statusCode = 500;
    ...
    server.listen(1455, "127.0.0.1", () => {
      ...
      if (!info || typeof info === "string") {
        reject(new RaxodeConfigError("Unable to determine local OAuth callback port."));
        server.close();
        return;
      }
      port = info.port;
...
async function requestDeviceCode(issuer = OPENAI_OAUTH_ISSUER, clientId = OPENAI_OAUTH_CLIENT_ID): Promise<Required<OpenAIDeviceCodeResponse>> {
  ...
  if (!response.ok) {
    const detail = await response.text();
    throw new RaxodeConfigError(`OpenAI device auth request failed: ${response.status} ${detail || response.statusText}`);
  }
  const payload = await response.json() as OpenAIDeviceCodeResponse;
  if (!payload.device_auth_id || !payload.user_code) {
    throw new RaxodeConfigError("OpenAI device auth did not return a device_auth_id and user_code.");
  }
  return {
    ...payload,
  ...
  while (Date.now() < deadline) {
    ...
    if (response.ok) {
      ...
      if (!payload.authorization_code || !payload.code_verifier) {
        throw new RaxodeConfigError("OpenAI device auth returned an incomplete authorization payload.");
      }
      return payload;
    }
    ...
    const detail = await response.text();
    throw new RaxodeConfigError(`OpenAI device auth polling failed: ${response.status} ${detail || response.statusText}`);
  }
  throw new RaxodeConfigError("OpenAI device auth timed out after 15 minutes.");
}
...
export async function refreshOpenAIOAuthIfNeeded(
  ...
  if (!refreshToken) {
    throw new RaxodeConfigError("OpenAI 官方登录缺少 refresh token，请重新登录。");
  }
  ...
  if (!tokens.access_token || !tokens.id_token) {
    throw new RaxodeConfigError("OpenAI token refresh returned incomplete tokens. Please log in again.");
  }
