...
function createWorkspaceBranchFamily(agentId: string): WorkspaceRaxodeGitReadback["branchFamily"] {
  const normalizedAgentId = normalizeAgentId(agentId);
  return {
    agentId: normalizedAgentId,
    workBranchName: `work/${normalizedAgentId}`,
    cmpBranchName: `cmp/${normalizedAgentId}`,
    mpBranchName: `mp/${normalizedAgentId}`,
    tapBranchName: `tap/${normalizedAgentId}`,
  };
}
...
export async function readWorkspaceRaxodeGitReadback(params: {
  ...
  const branchFamily = createWorkspaceBranchFamily(params.agentId);
  ...
  return {
    ...
    cmpWorktreePath: worktreeRootPath
      ? `${worktreeRootPath}/${branchFamily.cmpBranchName.replaceAll("/", "__")}`
      : undefined,
    checkedRefName,
    promotedRefName,
