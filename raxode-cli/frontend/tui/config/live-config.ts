...
function requireField(source: Record<string, string>, field: string): string {
  ...
  if (!value) {
    if (process.env.PRAXIS_LIVE_ENV_FILE) {
      throw new RaxodeConfigError(`显式环境配置缺少必填字段: ${field}`);
    }
    throw new RaxodeConfigError(
      `Raxode 全局配置未完成，缺少 ${field}。请编辑 ${resolveAuthJsonPath()} 和 ${resolveConfigJsonPath()}。`,
      { fieldPath: field },
    );
  }
  return value;
...
function applyResolvedProfileValues(
  ...
  profile: Pick<RaxodeResolvedProfile, "profile" | "authProfile">,
...
function readJsonOpenAIRoleValues(
  roleId: Parameters<typeof loadRaxodeRolePlan>[0],
...
function normalizeOpenAIConfigArgs(
  roleOrEnvPath?: Parameters<typeof loadRaxodeRolePlan>[0] | string,
  envPath?: string,
): { roleId: Parameters<typeof loadRaxodeRolePlan>[0]; envPath?: string } {
  if (typeof roleOrEnvPath === "string" && isRaxodeRoleId(roleOrEnvPath)) {
    return {
  ...
  return {
    roleId: (roleOrEnvPath as Parameters<typeof loadRaxodeRolePlan>[0] | undefined) ?? "core.main",
    envPath,
  };
}
...
export function loadOpenAILiveConfig(
  ...
  const rolePlan = loadRaxodeRolePlan(normalizedArgs.roleId);
  return buildOpenAILiveConfig(values, rolePlan);
}

function buildOpenAILiveConfig(
  values: Record<string, string>,
  rolePlan: ReturnType<typeof loadRaxodeRolePlan>,
): OpenAILiveConfig {
...
export function loadLiveProviderConfig(
  ...
  return {
    openai: {
      ...buildOpenAILiveConfig(values, loadRaxodeRolePlan("core.main")),
    },
    anthropic: {
